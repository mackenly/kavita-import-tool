...
- Perfered Format: This selector allows you to pick a perffered format. When you import files with the same name, but different formats, this options chooses which one to actually use. By default the order is `epub` and `pdf`, however, it will fall back to any file if not one of those.
